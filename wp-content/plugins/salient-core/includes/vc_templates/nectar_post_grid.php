...
if( !empty($cat_links_escaped) ) {
	echo '<div class="'.$filter_el_css_class.'" data-active-color="'.esc_attr($sortable_color).'" data-align="'.esc_attr($sortable_alignment).'" data-animation="'.esc_attr($animation).'" data-sortable="'.esc_attr($enable_sortable).'">';
	if ( 'yes' === $enable_sortable ) {
		echo '<h4>'.esc_html__('Filter','salient-core').'</h4>';
	}
	echo '<div>'.$cat_links_escaped.'</div></div>';
}
